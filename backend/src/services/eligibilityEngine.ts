...
export class EligibilityEngine {
    ...
    public static evaluate(profile: CitizenProfile, schemes: any[]): any {
        console.log(`\n🔍 Evaluating eligibility for: ${profile.full_name} (${profile.citizen_id})`);
        ...
        console.log(`❌ Ineligible for ${ineligible.length} schemes.`);
    ...
    private static evaluateScheme(profile: CitizenProfile, scheme: any): EligibilityResult {
        ...
        if (!rules) {
            console.warn(`⚠️ No engine rules found for scheme ${scheme.scheme_id}`);
            return {
        ...
        for (const rule of (rules.exclusion_rules || [])) {
            if (this.evaluateRule(profile, rule)) {
                ...
                console.log(`   [${scheme.scheme_id}] Exclusion matched: ${rule.rule_id}`);
            }
        }
        ...
        for (const rule of (rules.inclusion_rules || [])) {
            if (this.evaluateRule(profile, rule)) {
                ...
                // Check if it's a document related rule to mark as missing
   ...
   * Enhanced to search common categories if top-level path fails
   */
    private static getNestedValue(obj: any, path: string): any {
        ...
        // 2. If not found and not a dot path, try searching in common categories
        if (!path.includes('.')) {
            const categories = [
